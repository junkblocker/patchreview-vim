...
function! s:perforce.detect() " {{{
  try
    ...
    call s:driver.echo('Exception ' . v:exception)
    call s:driver.echo('From ' . v:throwpoint)
    return 0
  endtry
endfunction
" }}}
...
function! s:perforce.get_diff() " {{{
  ...
  while l:line_num < l:linescount
    ...
    call s:driver.log('Processing ' . l:line)
    let l:line_num += 1
    ...
    if l:line =~ '\(delete \(default \)\?change\) .*\(text\|unicode\|utf16\)'
      call s:driver.log('Fetching original ' . l:fwhere)
      let l:diff += ['--- ' . l:fwhere]
      let l:diff += ['+++ /dev/null']
    ...
    elseif l:line =~ '\(\(add\|branch\) \(default \)\?change\) .*\(text\|unicode\|utf16\)'
      call s:driver.log('Reading ' . l:fwhere)
      let l:diff += ['--- /dev/null']
      let l:diff += ['+++ ' . l:fwhere]
    ...
    elseif l:line =~ '\(\(edit\|integrate\) \(default \)\?change\) .*\(text\|unicode\|utf16\)'
      call s:driver.log('Diffing ' . l:fwhere)
      let l:diff += ['--- ' . l:fwhere]
      let l:diff += ['+++ ' . l:fwhere]
