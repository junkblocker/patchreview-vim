...
function! s:git.get_diff() " {{{
  if has('win16') || has('win32') || has('win64') || has('win95') || has('gui_win32') || has('gui_win32s')
    let l:diff = s:driver.generate_diff('git diff -p -U5 --no-color --no-renames 2> NUL')
  else
    let l:diff = s:driver.generate_diff('git diff -p -U5 --no-color --no-renames 2>/dev/null')
  endif
